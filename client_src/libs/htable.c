...
void grow_table(htable *htb)
{
   ...
   for (item=htable_get_first(htb); item; ) {
      ...
      htable_insert(big,((hlink *)((char *)item+htb->loffset))->key, item);
      if (ni) {
...
*/
void cut_table(htable *this){
	void *item;
	hlink *freeptr = NULL;
	int count = 0;
   	for (item = htable_get_first(this); item; ) {/*遍历hash表*/
		hlink *ni,*li;
		li = (hlink *)((char *)item+this->loffset);
		ni = li->next; 
		if(ni){
			count++;
			if(count <= 2)
		      	item = (void *)((char *)ni-this->loffset);	
			else{
				freeptr = ni;
				li->next = NULL;
				this->walkptr = NULL;
				item = htable_get_next(this);
				count = 0;
				if(freeptr){
					hlink *ni2,*li2;
					li2 = ni2 = freeptr;
					do{/*释放删除条目的内存*/
						ni2 = li2->next;
						free((char *)((char *)li2-this->loffset));
						this->num_items--;
						li2 = ni2;
					}while(li2);			
					freeptr = NULL;
...
void htable_destroy(htable *tbl)
{
   ...
   void *ni;
   void *li = htable_get_first(tbl);
   do {
      ni = htable_get_next(tbl);
      free((char *)li);
      li = ni;
   } while (ni);
...
void finger_chunk(unsigned char *buf, uint32_t len,unsigned char hash[])
{
   SHA_CTX context;
